<script>
    ...
    $(document).ready(function(){
        $(".ch-pagination a").click(function(e){
            ...
            if(offset != undefined){
                var url = "<?php echo base_url('users/index/dosen')?>"+"/"+offset;
                loadData(url);
            }
        })
        ...
        $("#formSearch").submit(function(e){
            e.preventDefault;
            loadData("<?php echo base_url('users/index/dosen/0');?>");
            return false;
        })
        ...
        $("table#users_data tbody tr").contextMenu({
            ...
            var act = action;
            if(act=='add-user'){
                document.location = "<?php echo base_url('users/add/dosen');?>";
            }else if(act=='update-user'){
                document.location = "<?php echo base_url('users/update/dosen');?>"+"/"+$(el).attr('id');
            } else if(act=='change'){
...
<table id="users_data"class="table table-striped table-hover table-condensed ch-table">
    ...
    ?>
        <tr id="<?php echo $user['user_id'];?>" class="<?php $status = ($user['user_status']==1) ? "aktif" : "non-aktif"; echo $status?>">
            ...
            <td><?php echo $user['dsn_alamat'];?></td>
        </tr>
    <?php endforeach ?>
    </tbody>
</table>
